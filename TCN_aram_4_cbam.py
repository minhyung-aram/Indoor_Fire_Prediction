...
class ChannelAttention(nn.Module):
    ...
    def __init__(self, channels, reduction_ratio=16):
        ...
        # 전역 평균 풀링과 전역 최대 풀링 적용 후 공유 MLP 통과
...
class SpatialAttention(nn.Module):
    """
    공간 어텐션 모듈 (CBAM의 두 번째 부분) - 시간 축에 dilation 적용
    """
    def __init__(self, kernel_size=7, time_dilation=1):
        super(SpatialAttention, self).__init__()
        
        # 시간 축에 dilation 적용을 위한 패딩 계산
        spatial_padding = kernel_size // 2
        time_padding = ((kernel_size-1) * time_dilation) // 2
        
        # 채널 차원의 평균과 최대값을 연결한 후 컨볼루션 적용
        self.conv = nn.Conv3d(
            2, 1, 
            kernel_size=(kernel_size, kernel_size, kernel_size),
            padding=(time_padding, spatial_padding, spatial_padding),
...
class CBAM(nn.Module):
    """
    전체 CBAM 모듈: 채널 어텐션 다음에 공간 어텐션 적용
    """
    def __init__(self, channels, reduction_ratio=16, spatial_kernel_size=7, time_dilation=1):
        super(CBAM, self).__init__()
        
        self.channel_attention = ChannelAttention(channels, reduction_ratio)
        self.spatial_attention = SpatialAttention(spatial_kernel_size, time_dilation)
...
# ResidualBlock에서 SE 블록 대신 CBAM 사용하는 예시
class ResidualBlockWithCBAM(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation, dropout_tcn=0.3):
        ...
        # SE 블록 대신 CBAM 사용 - TCN과 동일한 dilation 적용
        self.cbam1 = CBAM(out_channels, reduction_ratio=16, spatial_kernel_size=kernel_size, time_dilation=dilation)
        self.cbam2 = CBAM(out_channels, reduction_ratio=16, spatial_kernel_size=kernel_size, time_dilation=dilation)
...
# 기존 TemperatureTCN 클래스에서 ResidualBlock을 ResidualBlockWithCBAM으로 교체하는 예시
...
def train_model(model, train_dataloader, valid_dataloader, num_epochs=300, resume_path=None):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, './TCN_final_checkpoints/real_please_cbam.pth')
