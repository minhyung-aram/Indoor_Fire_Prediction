...
class ECABlock(nn.Module):
    """
    Efficient Channel Attention Block for 5D inputs
    논문: ECA-Net: Efficient Channel Attention for Deep Convolutional Neural Networks
    """
    def __init__(self, channels, gamma=2, b=1):
        ...
        k_size = max(t if t % 2 else t + 1, 3)  # 홀수로 만들고 최소 3
        
        # 3D 데이터에 대한 전역 평균 풀링
        self.avg_pool = nn.AdaptiveAvgPool3d(1)
        
        # 1D 컨볼루션으로 채널 간 관계 모델링 (더 효율적)
        self.conv = nn.Conv1d(
            in_channels=1, 
            out_channels=1, 
            kernel_size=k_size, 
    ...
    def forward(self, x):
        ...
        y = self.avg_pool(x).view(batch_size, 1, channels)
        
        # 1D 컨볼루션을 이용한 효율적인 채널 어텐션
        y = self.conv(y)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation):
        ...
        self.eca2 = ECABlock(out_channels)
...
def train_model(model, train_dataloader, valid_dataloader, num_epochs=300, resume_path=None):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, (data, label) in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{num_epochs}")):
            ...
            # 총 손실
            total_loss =  0.4 * temp_loss + 0.5 * co_loss + 0.1 * soot_loss
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, './TCN_final_checkpoints/test_4_layer_eca.pth')
